...
type ComplementoInstitucion struct {
	...
	ComplementoId *Complemento `orm:"column(complemento_id);rel(fk)"`
	JornadaId     *Jornada     `orm:"column(jornada_id);rel(fk)"`
	Cantidad      int          `orm:"column(cantidad);null"`
	InstitucionId *Institucion `orm:"column(institucion_id);rel(fk)"`
	RangoEdadId   *RangoEdad   `orm:"column(rango_edad_id);rel(fk)"`
}
